...
def upsert_customer(con: sqlite3.Connection, domain: str, brand=None, canonical_root=None) -> int:
    cur = con.execute("SELECT id FROM customers WHERE domain=?", (domain,))
    row = cur.fetchone()
    if row: return row[0]
    con.execute("INSERT INTO customers(domain,brand,canonical_root) VALUES(?,?,?)",
                (domain, brand, canonical_root))
    return con.execute("SELECT last_insert_rowid()").fetchone()[0]

def insert_link(con: sqlite3.Connection, cid: int, pub_domain, target_domain, target_url, anchor_text, source_url=None, campaign=None):
    con.execute("""INSERT INTO links(customer_id,pub_domain,target_domain,target_url,anchor_text,source_url,campaign)
                   VALUES(?,?,?,?,?,?,?)""",
