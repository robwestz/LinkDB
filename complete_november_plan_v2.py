...
MANUAL_CUSTOMER_DATA = {
    'Vera&John': {
        'domain': 'verajohn.com',
        'target_urls': ['https://www.verajohn.com/sv', 'https://www.verajohn.com/sv/casino', 'https://www.verajohn.com/sv/live-casino'],
        'anchors': ['Vera&John', 'Vera&John casino', 'casino online', 'spela casino', 'casinospel']
    },
    'Lucky Casino': {
    ...
    'Fair Investments': {
        ...
        'target_urls': ['https://fairinvestments.se', 'https://www.linkedin.com/company/fair-investments-sweden-ab'],
        'anchors': ['Fair Investments', 'investering', 'kapitalförvaltning', 'finansiella tjänster', 'investeringsbolag']
    },
    'Zmarta Sverige': {
        'domain': 'zmarta.se',
        'target_urls': ['https://www.zmarta.se/', 'https://www.zmarta.se/lana-pengar', 'https://www.zmarta.se/lana-pengar/privatlan'],
        'anchors': ['Zmarta', 'jämför lån', 'privatlån', 'låna pengar', 'lånekalkylator', 'bästa lånen']
    },
    'Florister I Sverige': {
        'domain': 'florister.se',
        'target_urls': ['https://florister.se', 'https://florister.se/blomsterbutiker'],
        'anchors': ['Florister i Sverige', 'blomsterbutik', 'beställ blommor', 'blomsterleverans']
    },
    'D-Bet': {
        'domain': 'dbet.com',
        'target_urls': ['https://www.dbet.com/', 'https://www.dbet.com/sports/', 'https://www.dbet.com/sportnyheter-och-sponsorskap/3/dbet-huvudsponsor-handbollsligan-2025/'],
        'anchors': ['D-Bet', 'betting online', 'sportbetting', 'odds', 'livebetting']
    },
    'Nordic Knots': {
        'domain': 'nordicknots.com',
        'target_urls': ['https://www.nordicknots.com/us/rugs', 'https://www.nordicknots.com/us/custom-rugs'],
        'anchors': ['Nordic Knots', 'handmade rugs', 'custom rugs', 'scandinavian rugs']
    },
    'Epidemic Sound': {
        'domain': 'epidemicsound.com',
        'target_urls': ['https://www.epidemicsound.com/music/', 'https://www.epidemicsound.com/es/music'],
        'anchors': ['Epidemic Sound', 'royalty-free music', 'music library', 'background music', 'stock music']
    }
}
...
for idx, row in df.iterrows():
    ...
    if kund in MANUAL_CUSTOMER_DATA:
        ...
        if needs_target:
            ...
            print(f"✓ Rad {idx}: {kund} → målsida: {manual_data['target_urls'][target_idx]} (manuell)")
            completed_targets += 1
        ...
        if needs_anchor:
            ...
            print(f"✓ Rad {idx}: {kund} → ankar: '{manual_data['anchors'][anchor_idx]}' (manuell)")
            completed_anchors += 1
    ...
    if not customer:
        print(f"⚠️  Rad {idx}: Kund '{kund}' varken i databas eller manuell mappning - använder generisk")
        
        if needs_target:
            domain_guess = kund.lower().replace(' ', '').replace('&', '')
            if not any(domain_guess.endswith(ext) for ext in ['.com', '.se', '.io', '.ai']):
                domain_guess += '.com'
    ...
    if needs_target:
        ...
        if common_targets:
            ...
            print(f"✓ Rad {idx}: {kund} → målsida: {common_targets[target_idx]['target_url']}")
            completed_targets += 1
        else:
    ...
    if needs_anchor:
        ...
        if common_anchors:
            ...
            print(f"✓ Rad {idx}: {kund} → ankar (fallback): '{customer['brand'] or kund}'")
            completed_anchors += 1
