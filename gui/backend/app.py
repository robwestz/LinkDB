...
DB_PATH = str(
    Path(__file__).parent.parent.parent / "data" / "output" / "linkops_history.db"
)
...
def get_dashboard_metrics():
    ...
    try:
        ...
        top_performers = [
            {"domain": row[0], "links": row[1]} for row in cursor.fetchall()
        ]
...
def get_competitive_overview():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error fetching competitive overview: {str(e)}"
        )
...
def get_customer_analysis(customer_id: int):
    ...
    try:
                ...
                "anchor_quality": (
                    {
                        ...
                        "commercial_keywords_ratio": round(
                            anchor.commercial_keywords_ratio * 100, 1
                        ),
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Database not found at {DB_PATH}. Please ensure the path is correct.",
        )
    except Exception as e:
