...
def get_anchor_frequency(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching anchor frequency: {str(e)}")
...
def get_word_frequency(
    ...
    try:
        ...
        top_words = [
            {
                "word": word,
                "frequency": count,
                "percentage": round((count / total_anchor_texts * 100), 2) if total_anchor_texts > 0 else 0
            }
        ...
        raise HTTPException(status_code=500, detail=f"Error fetching word frequency: {str(e)}")

@router.get("/customers/{customer_id}/anchor-search")
def search_anchor_texts(
    customer_id: int,
    search_term: str = Query(..., min_length=1, description="Word or phrase to search for"),
    from_date: Optional[str] = Query(None, description="Start date (YYYY-MM)"),
    to_date: Optional[str] = Query(None, description="End date (YYYY-MM)"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error searching anchor texts: {str(e)}")
...
def get_target_url_analysis(
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "statistics": {
                    ...
                    "deep_linking_ratio": round((deep_links / total_links * 100), 2) if total_links > 0 else 0,
                    "date_range": {
                        "from": from_date,
                        "to": to_date
                    }
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing target URLs: {str(e)}")
...
def get_link_velocity(
    ...
    try:
        ...
        if len(monthly_data) >= 2:
            ...
            total_change = monthly_data[-1]['link_count'] - monthly_data[0]['link_count']
            ...
            # Calculate average
            avg_links_per_month = sum(m['link_count'] for m in monthly_data) / len(monthly_data)
        else:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing link velocity: {str(e)}")
...
def get_domain_sources(
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "statistics": {
                    ...
                    "new_domain_ratio": round((new_domains / len(domains) * 100), 2) if len(domains) > 0 else 0,
                    "date_range": {
                        "from": from_date,
                        "to": to_date
                    }
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing domain sources: {str(e)}")
    ...
    try:
        ...
        changes = {
            ...
            "total_links_change_percent": round(
                ((period2['total_links'] - period1['total_links']) / period1['total_links'] * 100) if period1['total_links'] > 0 else 0,
                2
            ),
            "unique_domains_change": period2['unique_domains'] - period1['unique_domains'],
            "unique_domains_change_percent": round(
                ((period2['unique_domains'] - period1['unique_domains']) / period1['unique_domains'] * 100) if period1['unique_domains'] > 0 else 0,
                2
            ),
            "unique_urls_change": period2['unique_target_urls'] - period1['unique_target_urls'],
            "unique_urls_change_percent": round(
                ((period2['unique_target_urls'] - period1['unique_target_urls']) / period1['unique_target_urls'] * 100) if period1['unique_target_urls'] > 0 else 0,
                2
            )
        }
