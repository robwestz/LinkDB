...
def import_sheets():
    ...
    try:
        ...
        for idx, row in enumerate(
            all_rows, start=2
        ):  # start=2 eftersom rad 1 är headers
            pub_domain = row.get("Domain", "").strip()
            customer_name = row.get("Kund", "").strip()

            # Försök hitta target URL i olika kolumner
            target_url = (
                row.get("Targeting URL", "").strip()
                or row.get("IsCustomer", "").strip()
                or row.get("Target URL", "").strip()
            )
            ...
            if customer:
                ...
                skipped_rows.append(
                    f"Rad {idx}: Kund '{customer_name}' hittades inte i databasen"
                )
        ...
        if skipped_rows:
            response["skipped_rows"] = skipped_rows[
                :10
            ]  # Max 10 för att inte överväldiga
            response["warning"] = (
                f"{len(skipped_rows)} rader skippades. Se detaljer nedan."
            )
...
def load_preflight():
    ...
    try:
        ...
        for idx, row in enumerate(all_rows, start=2):  # start=2 för att matcha Excel
            ...
            if client_info:
                ...
                validation_errors.append(
                    f"Rad {idx}: Kund '{customer_name}' saknas i client_index"
                )
        ...
        for cid in customer_summary:
            customer_summary[cid]["pub_domains"] = list(
                customer_summary[cid]["pub_domains"]
            )
    ...
    try:
        ...
        for customer in active_customers:
            ...
            for i in range(suggested_count):
                ...
                pub_domain = (
                    common_pubs[i % len(common_pubs)]["pub_domain"]
                    if common_pubs
                    else None
                )
                ...
                target_url = (
                    common_targets[i % len(common_targets)]["target_url"]
                    if common_targets
                    else None
                )
                ...
                anchor_text = (
                    common_anchors[i % len(common_anchors)]["anchor_text"]
                    if common_anchors
                    else None
                )
        ...
        return jsonify(
            {
                ...
                "corrections": (
                    corrections[:10] if corrections else []
                ),  # Max 10 för att inte överväldiga
            }
        )
...
def semantic_preflight():
    ...
    try:
        ...
        if client_index_path.exists():
            ...
            for _, row in client_index.iterrows():
                if pd.notna(row.get("customer_id")) and pd.notna(
                    row.get("client_domain")
                ):
                    client_domains[int(row["customer_id"])] = row[
                        "client_domain"
                    ].strip()
            ...
            for key, item in raw_planning_data.items():
                ...
                if not client_domain:
                    url_validation["warnings"].append(
                        f"Kund ID {customer_id} saknas i client_index"
                    )
                    continue
        ...
        for key, item in raw_planning_data.items():
            if isinstance(item, dict):
                ...
                if cid not in customer_aggregation:
                    customer_aggregation[cid] = {"customer_id": cid, "links": []}
        ...
        for cid, agg in customer_aggregation.items():
            ...
            if semantic_possible:
                ...
                recommended_clusters = [
                    t["anchor_text"][:20] for t in topics if t["anchor_text"]
                ]
            ...
            customer_analyses.append(
                {
                    ...
                    "recommended_clusters": (
                        recommended_clusters if recommended_clusters else None
                    ),
                }
            )
            ...
            if semantic_possible:
                recommendations.append(
                    f"{customer['canonical_root']}: Kan bygga semantiska kluster"
                )
            if can_build_authority:
                recommendations.append(
                    f"{customer['canonical_root']}: Kan bygga topical authority"
                )
            if planned_links < 6:
                recommendations.append(
                    f"{customer['canonical_root']}: Överväg fler länkar för semantisk analys"
                )
        ...
        return jsonify(
            {
                ...
                "topical_authority_opportunities": sum(
                    1 for ca in customer_analyses if ca["can_build_authority"]
                ),
    ...
    try:
        ...
        for key, item in raw_planning_data.items():
            if isinstance(item, dict):
                ...
                planning_data[customer_id] = (
                    planning_data.get(customer_id, 0) + link_count
                )
        ...
        ai_prompt = generate_ai_prompt(
            raw_planning_data, planning_data, ai_instructions
        )
...
def generate_ai_prompt(raw_planning_data, planning_data, ai_instructions):
    ...
    prompt_parts.append("")
    prompt_parts.append(
        "Du är en expert på SEO och länkbyggnad med djup förståelse för semantisk SEO och topical authority."
    )
    prompt_parts.append("")
    prompt_parts.append("## 📋 UPPGIFT:")
    prompt_parts.append(
        "Generera en intelligent länkplan baserat på nedanstående data. För varje länk ska du:"
    )
    prompt_parts.append(
    ...
    for key, item in raw_planning_data.items():
        if isinstance(item, dict):
            ...
            if cid not in customer_data:
                ...
                customer_data[cid] = {"customer": customer, "items": []}
    ...
    for cid, data in customer_data.items():
        ...
        prompt_parts.append(
            f"### Kund: {customer['canonical_root']} (Brand: {customer['brand']})"
        )
        prompt_parts.append(
            f"- **Totalt länkar:** {sum(item['link_count'] for item in items)}"
        )
        prompt_parts.append(
            f"- **Rekommenderad strategi:** {get_strategy_for_count(sum(item['link_count'] for item in items))}"
        )
        prompt_parts.append("")

        # Hämta historisk data
        history_count = con.execute(
            "SELECT COUNT(*) as cnt FROM links_history WHERE customer_id = ?", (cid,)
        ).fetchone()["cnt"]
        ...
        if common_anchors:
            ...
            for anchor in common_anchors:
                prompt_parts.append(
                    f"  - \"{anchor['anchor_text']}\" ({anchor['cnt']}x)"
                )
        ...
        for i, item in enumerate(items, 1):
            prompt_parts.append(
                f"{i}. **{item['link_count']} länkar** på **{item['pub_domain']}**"
            )
            if item.get("target_url"):
...
def self_correct_planning(suggestions, con):
    ...
    for suggestion in suggestions:
        ...
        unique_targets = len(
            set(l["target_url"] for l in corrected_links if l["target_url"])
        )
        unique_anchors = len(
            set(l["anchor_text"] for l in corrected_links if l["anchor_text"])
        )
        ...
        corrected_suggestions.append(
            {
                ...
                "quality_metrics": {
                    ...
                    "variation_score": round(
                        (unique_pubs + unique_targets + unique_anchors)
                        / (total_links * 3)
                        * 100,
                        1,
                    ),
            ...
            customer_analyses.append(
                {
                    ...
                    "needs_anchor": sum(1 for l in links if l["needs_anchor"]),
                    "needs_both": sum(
                        1 for l in links if l["needs_target"] and l["needs_anchor"]
                    ),
        ...
        return jsonify(
            {
                ...
                "total_needs_target": sum(
                    c.get("needs_target", 0) for c in customer_analyses
                ),
                "total_needs_anchor": sum(
                    c.get("needs_anchor", 0) for c in customer_analyses
                ),
            }
        )
...
def generate_customer_specific_agent_prompt(customer_data):
    ...
    prompt_parts.append(
        f"- **Canonical Root:** {customer_data.get('canonical_root', 'N/A')}"
    )
    prompt_parts.append(f"- **Brand:** {customer_data.get('brand', 'N/A')}")
    prompt_parts.append(
        f"- **Marknader:** {', '.join(customer_data.get('markets', ['SE']))}"
    )
    prompt_parts.append("")

    prompt_parts.append("## 🎯 UPPGIFT:")
    prompt_parts.append(
        f"Du ska komplettera **{customer_data['total_links']} länkar** för november 2025:"
    )
    prompt_parts.append(f"- **{customer_data['needs_target']} länkar** behöver målsida")
    prompt_parts.append(
        f"- **{customer_data['needs_anchor']} länkar** behöver ankartext"
    )
    prompt_parts.append(
        f"- **{customer_data.get('needs_both', 0)} länkar** behöver både målsida och ankartext"
    )
    prompt_parts.append("")
    ...
    if customer_data.get("common_anchors"):
        ...
        for anchor in customer_data["common_anchors"][:10]:
            prompt_parts.append(
                f"- \"{anchor['anchor_text']}\" ({anchor['cnt']}x använd)"
            )
        prompt_parts.append("")
    ...
    prompt_parts.append(
        "1. **Målsidor:** Välj från historiska målsidor eller skapa nya som matchar kundens domän"
    )
    prompt_parts.append(
        "2. **Ankartexter:** Skapa naturliga, varierade ankartexter (exact/partial/branded/LSI)"
    )
    prompt_parts.append(
        "3. **Variation:** Undvik att upprepa samma ankartext för många gånger"
    )
    prompt_parts.append(
        "4. **Marknad:** Anpassa språk efter marknad (SE=svenska, UK/US=engelska, DK=danska)"
    )
    ...
    try:
        ...
        if search:
            if column and column in df.columns:
                ...
                mask = (
                    df.astype(str)
                    .apply(lambda x: x.str.lower().str.contains(search, na=False))
                    .any(axis=1)
                )
                df_filtered = df[mask]
        else:
        ...
        unique_customers = (
            df_filtered["canonical_root"].nunique()
            if "canonical_root" in df_filtered.columns
            else 0
        )
